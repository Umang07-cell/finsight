...
export default function Chat() {
  ...
  return (
    ...
    >
      ...
      {/* Top bar */}
      <div style={{ backgroundColor: topBarBg, borderColor }} className="backdrop-blur-sm border-b z-10 px-3 sm:px-6 py-3 sm:py-4">

        {/* Desktop */}
        <div className="hidden sm:flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-shrink-0">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setDrawerOpen(!drawerOpen)} className="w-9 h-9 flex flex-col gap-1.5 items-center justify-center rounded-xl" style={{ color: textColor }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <span style={{ color: textColor }} className="font-semibold text-base whitespace-nowrap">FinSight</span>
            </div>
          </div>

          <div style={{ backgroundColor: darkMode ? '#1e293b' : '#f1f5f9', borderColor }} className="flex items-center rounded-xl p-1 gap-1">
            {Object.entries(themes).map(([key]) => (
              <motion.button key={key} onClick={() => setMode(key)} whileTap={{ scale: 0.95 }}
                ...
                className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-300 shadow-sm whitespace-nowrap">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setDarkMode(!darkMode)} style={{ borderColor, color: subTextColor }} className="text-xs border px-3 py-1.5 rounded-lg whitespace-nowrap">
              {darkMode ? '☀️ Light' : '🌙 Dark'}
            </motion.button>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={clearMessages} style={{ borderColor, color: subTextColor }} className="text-xs border px-3 py-1.5 rounded-lg whitespace-nowrap">
              Clear chat
            </motion.button>
          </div>
        </div>
        ...
        <div className="flex sm:hidden flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => setDrawerOpen(!drawerOpen)} className="w-8 h-8 flex flex-col gap-1.5 items-center justify-center rounded-xl" style={{ color: textColor }}>
                <span style={{ backgroundColor: textColor }} className="w-5 h-0.5 rounded block"></span>
                <span style={{ backgroundColor: textColor }} className="w-5 h-0.5 rounded block"></span>
                <span style={{ backgroundColor: textColor }} className="w-3 h-0.5 rounded block"></span>
              </motion.button>
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-gray-900 flex items-center justify-center"><span className="text-white font-bold text-xs">F</span></div>
                <span style={{ color: textColor }} className="font-semibold text-lg whitespace-nowrap">FinSight</span>
              </div>
            </div>
            <div className="flex items-center gap-1">
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => setDarkMode(!darkMode)} style={{ borderColor, color: subTextColor }} className="text-[10px] border px-2 py-1 rounded-lg">
                {darkMode ? '☀️' : '🌙'}
              </motion.button>
              <motion.button whileTap={{ scale: 0.95 }} onClick={clearMessages} style={{ borderColor, color: subTextColor }} className="text-[10px] border px-2 py-1 rounded-lg">
                Clear
              </motion.button>
            </div>
          </div>
      ...
      <AnimatePresence>
        {drawerOpen && (
          <>
            ...
            <motion.aside initial={{ x: -320 }} animate={{ x: 0 }} exit={{ x: -320 }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} style={{ backgroundColor: cardBg, borderColor }} className="fixed left-0 top-0 h-full w-80 shadow-2xl z-30 flex flex-col border-r">
              ...
              <div style={{ borderColor }} className="p-5 border-b">
                ...
                <input type="text" placeholder="Company name (e.g. Apple)" value={company} onChange={e => setCompany(e.target.value)} style={{ backgroundColor: darkMode ? '#0f172a' : '#f9fafb', borderColor, color: textColor }} className="w-full border rounded-xl px-3 py-2 text-sm mb-2 focus:outline-none" />
                <input type="text" placeholder="Year (e.g. 2023)" value={year} onChange={e => setYear(e.target.value)} style={{ backgroundColor: darkMode ? '#0f172a' : '#f9fafb', borderColor, color: textColor }} className="w-full border rounded-xl px-3 py-2 text-sm mb-3 focus:outline-none" />
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleCompanyInsight} disabled={!company || !year} className="w-full bg-gray-900 text-white py-2.5 rounded-xl text-sm font-medium disabled:opacity-40">
                  Analyze Company →
                </motion.button>
              </div>
              ...
              <div style={{ borderColor }} className="p-5 border-b">
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => fileRef.current.click()} disabled={uploading || !company || !year} style={{ borderColor, color: subTextColor }} className="w-full border-2 border-dashed rounded-xl py-3 text-sm disabled:opacity-40 transition-colors">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl w-full">
                {quickPrompts.map((q, i) => (
                  <motion.button key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i }} whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} onClick={() => handleSend(q)} style={{ backgroundColor: cardBg, borderColor, color: textColor }} className="border rounded-2xl px-4 py-3 text-sm text-left hover:shadow-sm transition-all">
                    {q}
                  </motion.button>
                ))}
              </div>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} style={{ backgroundColor: t.badge, color: t.badgeText }} className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start mb-4">
                <div style={{ backgroundColor: cardBg, borderColor }} className="border rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm">
                  <div className="flex gap-1">
                    {[0, 1, 2].map(i => (
                      <motion.div key={i} animate={{ y: [0, -5, 0] }} transition={{ duration: 0.5, repeat: Infinity, delay: i * 0.12 }} style={{ backgroundColor: t.button }} className="w-2 h-2 rounded-full" />
                    ))}
                  </div>
                </div>
              </motion.div>
      ...
      {selectedImage && (
        <div className="mx-4 mb-2 z-10 relative">
          <div style={{ backgroundColor: cardBg, borderColor }} className="border rounded-2xl p-3 flex items-center gap-3 shadow-lg">
            <img src={selectedImage.preview} className="w-12 h-12 rounded-xl object-cover flex-shrink-0" alt="preview" />
            ...
            <motion.button whileTap={{ scale: 0.95 }} onClick={() => { handleFileUpload(selectedImage.file); setSelectedImage(null) }} style={{ backgroundColor: t.button }} className="text-white px-3 py-1.5 rounded-lg text-xs flex-shrink-0">
              Analyze
            </motion.button>
          </div>
        </div>
      ...
      <div style={{ backgroundColor: topBarBg, borderColor }} className="p-4 backdrop-blur-sm border-t z-10">
        <div className="max-w-3xl mx-auto">
          <div style={{ backgroundColor: inputBg, borderColor }} className="flex items-end gap-2 border rounded-2xl px-3 py-2 shadow-sm">
            ...
            {/* PDF upload */}
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => fileRef.current.click()} style={{ color: subTextColor }} className="p-1 transition-colors flex-shrink-0" title="Upload PDF">
            ...
            {/* Image upload */}
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => imageRef.current.click()} style={{ color: subTextColor }} className="p-1 transition-colors flex-shrink-0" title="Upload Image">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey} placeholder="Ask anything about finance, markets, investments..." rows={1} style={{ color: textColor, caretColor: textColor }} className="flex-1 bg-transparent text-sm placeholder-gray-400 focus:outline-none resize-none py-1" />

            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handleSend()} disabled={loading || !input.trim()} style={{ backgroundColor: t.button }} className="text-white p-2 rounded-xl disabled:opacity-40 transition-colors flex-shrink-0">
